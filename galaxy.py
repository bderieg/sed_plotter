    ...
    def display_sed(self):
        ...
        for i in range(len(self.freq_list)):
            if any(ele == self.point_types[i] for ele in previous_points):
                plt.plot(self.freq_list[i], self.flux_list[i], color=self.point_colors[i], marker=self.point_types[i], markersize=9)
            else:
                pointlist[legend_counter], = plt.plot(self.freq_list[i], self.flux_list[i], color=self.point_colors[i],
                                                      marker=self.point_types[i], linestyle='None', markersize=9)
        ...
        plt.title('SED for ' + self.name, fontsize=20)
        plt.yscale('log')
        plt.xscale('log')
        plt.xlabel('Rest Frequency (Hz)', fontsize=15)
        plt.ylabel('Flux Density (Jy)', fontsize=15)
        plt.legend(pointlist, unique_tele_list, fontsize=15)
        ...
        x_axis_wl.tick_params(labelsize=15)
        x_axis_wl.set_xlabel("Rest Wavelength", fontsize=15)
